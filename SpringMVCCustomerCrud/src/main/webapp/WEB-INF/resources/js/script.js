$(document).ready(function() {
    ...
    function validateMobileNo() {
		...
		if (mobileNo === '') {
			...
			} else if(checkCondition === 'noCheck') {
				var originalMobileForUpdateProcess = $('#mobileno').val();  // get original value from hidden field to compare it with input
           	 	
           	 	if(mobileNo !== originalMobileForUpdateProcess) {
						checkMobileNoAvailability(mobileNo);   // Call AJAX function to check availability
				}
			}
        }
	...
	function validateEmail() {
		...
		if(email === '') {
            ...
            if(checkCondition === 'check') {
				...
				var originalEmailForUpdateProcess = $('#emailid').val();    // get original value from hidden field to compare it with input
   
           	 	if(email !== originalEmailForUpdateProcess) {
						checkEmailAvailability(email);       // Call AJAX function to check availability
				}
			}
		}
